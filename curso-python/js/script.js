...
const WHATSAPP_MESSAGE = "Hola, quiero mi cupo para el curso de Python para Análisis de Datos 🐍📊";

// Mensaje específico para los botones que apuntan a la promo de los primeros 5 estudiantes
const WHATSAPP_MESSAGE_PROMO = "Hola, quiero uno de los 5 cupos con descuento (S/99.90) del curso de Python para Análisis de Datos 🐍";

// Fecha y hora de inicio del curso (formato ISO). Se usa para el countdown del Hero.
// Sábado 5 de septiembre de 2026, 7:00 pm hora Perú (UTC-5)
const COURSE_START_DATE = new Date("2026-09-05T19:00:00-05:00");

// TODO: Cupos totales y cupos restantes disponibles. Edita CUPOS_RESTANTES según inscripciones reales.
// El grupo es de máximo 10 estudiantes por cohorte (clases 100% personalizadas con seguimiento).
const CUPOS_TOTALES = 10;
const CUPOS_RESTANTES = 8;

// TODO: Cupos con precio promocional de S/99.90 (primeros 5 estudiantes).
// Edita PROMO_RESTANTES conforme se vayan inscribiendo.
const PROMO_TOTALES = 5;
const PROMO_RESTANTES = 5;
...
function initWhatsAppLinks() {
  ...
  // CTAs que apuntan directamente a la promo de los primeros 5 estudiantes
  document.querySelectorAll("[data-wa-cta-promo]").forEach((el) => {
    el.href = waLink(WHATSAPP_MESSAGE_PROMO);
  });
...
function initCupos() {
  ...
  /* --- Cupos de la promo de los primeros 5 estudiantes --- */
  const promoRestantes = Math.max(Math.min(PROMO_RESTANTES, PROMO_TOTALES), 0);
  document.querySelectorAll("[data-promo-restantes]").forEach((el) => {
    el.textContent = promoRestantes;
  });
  document.querySelectorAll("[data-promo-totales]").forEach((el) => {
    el.textContent = PROMO_TOTALES;
  });

  const promoBarra = Math.min(
    100,
    Math.round(((PROMO_TOTALES - promoRestantes) / PROMO_TOTALES) * 100)
  );
  document.querySelectorAll("[data-promo-barra]").forEach((el) => {
    el.style.width = `${promoBarra}%`;
  });

  // Si ya se agotó la promo, se avisa en lugar de seguir prometiendo el descuento.
  if (promoRestantes === 0) {
    document.querySelectorAll("[data-promo-agotada]").forEach((el) => {
      el.classList.remove("hidden");
    });
    document.querySelectorAll("[data-promo-activa]").forEach((el) => {
